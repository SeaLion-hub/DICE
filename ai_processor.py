...
def extract_notice_info(body_text: str, title: Optional[str] = None) -> Dict[str, Any]:
    ...
    prompt = f"""
당신은 모든 유형의 대학 공지사항에서 구조화된 데이터를 추출하는 AI 전문가입니다.
주어진 제목과 본문을 깊이 분석하여, 다음 [핵심 추출 규칙]과 [JSON 출력 스키마]를 완벽하게 준수하여 결과를 출력하세요.

[핵심 추출 규칙]
1.  **가장 중요한 마감일 식별**: 공지에 '신청 기간', '서류 제출 기한', '행사 기간' 등 여러 날짜가 있다면, 학생이나 대상자의 행동이 요구되는 **'신청/접수/제출' 마감일을 'end_date'로 최우선 추출**하세요. 단순 행사일이나 교육 기간은 각각 start_date와 end_date로 설정할 수 있습니다. 모든 날짜는 **'YYYY-MM-DD'** 형식으로 변환하고, 특정할 수 없으면 **null**로 처리하세요.

2.  **자격 요건의 구체화 및 통합**: 본문과 제목에 흩어진 모든 대상 및 자격 요건(학년, 학과, 최소 학점, 신분)을 종합하여 JSON의 각 필드에 구조화하세요. '선발대상', '지원자격'과 같은 키워드를 중심으로 정보를 찾으세요.
    - '학부생 2~4학년'은 `grade: [2, 3, 4]`로, '대학원 재학생(석사, 박사)'은 `other: "대학원 재학생(석사, 박사)"`으로 명시하세요.
    - '박사 학위 소지자'와 같이 학생이 아닌 대상의 자격 조건도 `other` 필드에 정확히 기재하세요.

3.  **부정/제외 조건 강조**: **'~불가', '~제외', '결격사유'** 등 부정 조건은 필수 정보입니다. 절대 생략하지 말고 `qualification.other` 필드에 명확하게 요약하여 기재하세요.

4.  **정보 부재 처리**: '가이드라인 안내'처럼 **정보 추출 대상(날짜, 자격 등)이 없는 순수 정보성 공지**의 경우, 해당 필드를 **모두 null 또는 빈 값으로 처리**하세요. 절대 정보를 추측하거나 창작하지 마세요.

5.  **외부 정보 처리**: 만약 핵심 정보가 본문이 아닌 **'첨부 파일', '신청 링크', '홈페이지'**에 있다고 언급되면, `qualification.other` 필드에 **"상세 요건은 첨부 파일/링크 확인 필수"** 라고 반드시 명시하여 정보의 출처를 밝히세요.

[JSON 출력 스키마]
{{
  "category": "장학|채용|행사|수업|행정|기타|공모전/대회",
  ...
  "qualification": {{
    ...
    "min_gpa": 3.7,
    "major": ["사회과학대학"],
    "other": "[학부/대학원] 국가유공자 및 유족. 휴학생 지원 불가. 상세 자격은 첨부된 공고문 확인 필수."
  }}
}}

[입력 제목]: {title}
[입력 본문]: {(body_text or "")[:6000]}
""".strip()
